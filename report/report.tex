...
We explored three online reinforcement learning algorithms applied to the best T5-base SFT checkpoint (\autoref{tab:results_t5_base}, F1\,=\,85.96\%) via LoRA adapters ($r$\,=\,32, $\alpha$\,=\,64, targeting q/k/v/o projections, 3.08\% trainable parameters). This investigation was motivated by recent work on RL-based LLM alignment: \texttt{GRPO} from DeepSeekMath, \texttt{CISPO} from MiniMax-M1, and classic \texttt{PPO} with a learned value head baseline. To our knowledge, no published work applies these algorithms to T5 encoder-decoder models for NL-to-SQL.

\paragraph{Methodology.} For each query, $G$\,=\,4 SQL completions are generated via constrained decoding (temperature\,=\,0.7, top-$k$\,=\,50). Each completion receives a graded execution reward: +1.0 (exact record match), +0.5 (partial overlap), $-$0.5 (no overlap), $-$1.0 (SQL error). Advantages are computed group-relative for GRPO/CISPO and via a learned value head for PPO. Training uses a random subset of 200--500 queries per epoch with early stopping on dev Record F1.

\begin{itemize}
  \item \textbf{PPO}: Clipped surrogate objective with a learned value head (MLP on mean-pooled encoder hidden states) as baseline; entropy bonus ($\alpha$\,=\,0.01) for exploration.
  \item \textbf{GRPO}: PPO-style clipped surrogate with group-relative advantage normalization (no critic network). KL penalty ($\beta$\,=\,0.05).
  \item \textbf{CISPO}: Detached clamped importance sampling weights that preserve gradient flow for all tokens. No KL penalty ($\beta$\,=\,0).
\end{itemize}

\paragraph{Results.} \autoref{tab:rl_results} reports dev set metrics for all three algorithms.
...
\begin{tabular}{lccccc}
  ...
  \bottomrule
\end{tabular}
\caption{RL fine-tuning results on T5-base (Section~\ref{sec:rl}). All three algorithms match but do not improve the SFT baseline (85.96\% F1). KL Drift = KL divergence from the reference policy at early stopping.}
\label{tab:rl_results}
\end{table}

\paragraph{Analysis.} None of the three RL algorithms improved over the SFT baseline. We identify four contributing factors:

\begin{enumerate}
  \item \textbf{Reward sparsity.} The base model already achieves F1\,=\,85.96\%. Most generated completions receive +1.0 reward, creating 50--64\% ``dead groups'' (zero reward variance). The RL gradient signal is too sparse to improve an already-strong model.

  \item \textbf{Single-step ratio reduction.} With one gradient step per rollout batch, the importance sampling ratio is always 1.0. PPO clipping (clip\_fraction\,=\,0.000 across all runs) never activates. All three algorithms effectively reduce to vanilla REINFORCE with different advantage baselines.

  \item \textbf{Policy drift without improvement.} PPO and GRPO show increasing negative KL divergence ($-$6.55 and $-$9.88 by early stopping), meaning the policy explores lower-probability regions without finding better outputs. CISPO's detached clipping keeps KL near zero (+0.21) but makes effectively zero policy updates (loss $\approx -$0.008).

  \item \textbf{Coarse reward granularity.} Four discrete reward levels cannot distinguish ``almost correct'' from ``completely wrong'' SQL. A continuous reward based on record set Jaccard similarity might provide finer gradient signal.
\end{enumerate}

These results suggest that online RL is not effective for improving already-strong NL-to-SQL models on small datasets ($\sim$4K queries). The restricted 598-token SQL vocabulary already constrains the output space, limiting the model's ability to explore meaningfully different SQL structures. Rejection sampling fine-tuning (generate, filter by reward, SFT on filtered outputs) may be more practical for post-training improvement in this setting.
...
When reporting your results in \autoref{tab:results_llm}, you will need to refer to your ablations variants.
...
\makebox[\textwidth][c]{%
\begin{tabular}{>{\raggedright\arraybackslash}p{3cm}crccccccccccc}
  \toprule
...
\caption{T5-small development and test results (Parts 1 \& 2, trained on VM\,1). Params (M) = tunable parameters in millions. WD = weight decay, Beams = num\_beams during decoding. Best dev result in bold.}
...
\caption{T5-base fine-tuning development results from W\&B random sweep (222M total params, batch = 32, trained on VM\,2). WD = weight decay, Beams = num\_beams during decoding. Params (M) = tunable parameters in millions. Best result in bold.}
...
\caption{Training curves for two best fine-tuned models. \textbf{Left:} T5-small ``Restricted v3'' (full fine-tune, LR\,=\,3e-4, batch\,=\,64, schema augmentation, dropout\,=\,0.1, 79 epochs; see \autoref{tab:results_t5_small}). \textbf{Right:} T5-base ``Base sweep best'' (full fine-tune, LR\,=\,6.19e-5, batch\,=\,32, schema augmentation, dropout\,=\,0.05, label smoothing\,=\,0.15, 79 epochs; see \autoref{tab:results_t5_base}). Gray: training loss (left axis). Orange dashes: validation loss (left axis). Blue/red circles: Record F1 on a dev subset (right axis). Purple triangles (T5-base only): SQL Exact Match on a dev subset (right axis). Diamonds: final full dev set metrics. Both models converge within their time budgets, with T5-base achieving higher F1 (85.96\% vs.\ 79.60\%).}
...
\caption{Record F1 on the dev set across all T5-small variants. Full fine-tuning with schema augmentation (Restricted v3) achieves the highest F1 (79.6\%). LoRA with warm-start initialization (73.4\%) approaches full fine-tuning performance. MLP projection heads underperform at 8.7\%.}
...
\begin{tabular}{lccc}
  ...
  \bottomrule
\end{tabular}
\caption{DPO results on T5-base (Section~\ref{sec:dpo}). The SFT baseline is ``Base sweep best'' from \autoref{tab:results_t5_base}. \textbf{Full fine-tune:} training peaked at epoch 2 (F1\,=\,84.82\%) before degrading; early stopping triggered at epoch 7. The final beam-search evaluation of the best checkpoint (82.42\%) fell further below the SFT baseline (85.96\%). \textbf{LoRA:} the low learning rate (1.16e-7) caused effectively no policy update over 6 epochs---dev metrics remained identical to the SFT baseline. However, the final beam-search evaluation dropped to 75.75\%, a 10.2\,pp gap attributed to LoRA merge-and-save corruption and the greedy-vs-beam decoding mismatch (see \autoref{tab:dpo_issues}).}
...
\caption{ICL sensitivity to $k$ on the dev set. Random example selection (red) shows flat or slightly declining F1 as $k$ increases. BM25 per-query selection at $k=3$ (blue) achieves 17.35\%, a 45\% relative improvement over random $k=3$, demonstrating that retrieval-based example selection is critical for this task.}
...
\begin{table}
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{2.6cm}>{\raggedright\arraybackslash}p{1.1cm}l>{\raggedright\arraybackslash}p{5.5cm}p{5cm}>{\raggedright\arraybackslash}p{3.4cm}}
    \toprule
    \textbf{Error Type} & \textbf{Models} & \multicolumn{2}{c}{\textbf{Example Of Error}} & \textbf{Error Description} & \textbf{Statistics} \\
    \midrule
    \multirow[t]{3}{2.6cm}{Missing comparison operator}
    & \multirow[t]{3}{1.1cm}{T5-FT, T5-Scr}
    & NL   & ``flights arriving before 6pm''
    & \multirow[t]{3}{5cm}{Drops \texttt{<}, \texttt{>}, \texttt{<=}, \texttt{>=} from predicates, producing invalid SQL. SentencePiece treats angle brackets as special tokens.}
    & \multirow[t]{3}{3.4cm}{T5-FT: 26/466 (5.6\%) \newline T5-Scr: 27/466 (5.8\%)} \\
    & & GT   & \texttt{arrival\_time < 1800} & & \\
    & & Pred & \texttt{arrival\_time \space\space 1800} & & \\[8pt]
    \midrule
    \multirow[t]{3}{2.6cm}{Query truncation}
    & \multirow[t]{3}{1.1cm}{ICL}
    & NL   & ``afternoon flights from washington to boston''
    & \multirow[t]{3}{5cm}{Only a \texttt{SELECT} fragment generated, missing \texttt{FROM}/\texttt{WHERE}. Caused by Gemma 2B \texttt{max\_new\_tokens} limit.}
    & \multirow[t]{3}{3.4cm}{ICL: 383/466 (82.2\%)} \\
    & & GT   & \texttt{SELECT DISTINCT flight\_1.flight\_id FROM flight flight\_1 ...} & & \\
    & & Pred & \texttt{SELECT DISTINCT flight\_1.flight\_id} & & \\[8pt]
    \midrule
    \multirow[t]{3}{2.6cm}{Wrong table/column reference}
    & \multirow[t]{3}{1.1cm}{T5-FT, ICL}
    & NL   & ``flight from boston to denver''
    & \multirow[t]{3}{5cm}{References aliases or columns not in \texttt{FROM}. Model generates plausible patterns but omits table declarations.}
    & \multirow[t]{3}{3.4cm}{T5-FT: 16/466 (3.4\%) \newline ICL: 5/466 (1.1\%)} \\
    & & GT   & \texttt{...city city\_1, ...city city\_2} & & \\
    & & Pred & \texttt{...city city\_2} (missing \texttt{city\_1}) & & \\[8pt]
    \midrule
    \multirow[t]{3}{2.6cm}{Wrong JOIN structure}
    & \multirow[t]{3}{1.1cm}{T5-FT, T5-Scr, ICL}
    & NL   & ``flights from boston to SF with max stops''
    & \multirow[t]{3}{5cm}{Executes but returns wrong records: incorrect subqueries, missing joins, or wrong aggregation on multi-table queries.}
    & \multirow[t]{3}{3.4cm}{T5-FT: 83/466 (17.8\%) \newline T5-Scr: 144/466 (30.9\%) \newline ICL: 35/466 (7.5\%)} \\
    & & GT   & \texttt{stops = (SELECT MAX(stops)...)} & & \\
    & & Pred & \texttt{stops = 0} (returns 4 instead of 2) & & \\[8pt]
    \midrule
    \multirow[t]{3}{2.6cm}{Wrong predicate values}
    & \multirow[t]{3}{1.1cm}{T5-FT, T5-Scr, ICL}
    & NL   & ``flights available tomorrow from denver to philadelphia''
    & \multirow[t]{3}{5cm}{Correct SQL structure but wrong literals in \texttt{WHERE} (month, city, time). Subset of semantic errors, identified by matching \texttt{FROM}.}
    & \multirow[t]{3}{3.4cm}{T5-FT: 83/466 semantic \newline T5-Scr: 144/466 semantic \newline ICL: 35/466 semantic} \\
...
\bottomrule
\end{tabular}
\caption{DPO-specific engineering issues (Section~\ref{sec:dpo}).}
