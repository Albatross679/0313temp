...
def main():
    ...
    # Prepare examples / BM25 index
    examples = None
    bm25_index = None
    example_selection = getattr(cfg, "example_selection", "random")
    include_schema = getattr(cfg, "include_schema", True)
    include_instructions = getattr(cfg, "include_instructions", True)

    if cfg.shot > 0:
        if example_selection == "bm25":
            from part3.data import build_bm25_index
            bm25_index = build_bm25_index(train_x)
        else:
            examples = random.sample(list(zip(train_x, train_y)), cfg.shot)

    # W&B setup
    run_dir, wandb_run_id = setup_run(cfg, experiment_name="part3_llm_prompting")
    ...
    for eval_split in cfg.eval_splits:
        ...
        raw_outputs, extracted_queries = exp_kshot(
            tokenizer, model, eval_x, cfg.shot,
        ...
        if eval_y is not None:
            ...
            save_queries_and_records(extracted_queries, model_sql_path, model_record_path,
                                     num_threads=getattr(cfg, "sql_num_threads", 32))
