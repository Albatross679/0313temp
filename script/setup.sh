...
#
# Prerequisites:
#   - Docker image: ml-base (has all system + Python deps pre-installed)
...
if [[ -f "$ENV_FILE" ]]; then
    ...
    grep -v '^#' "$ENV_FILE" | grep '=' >> ~/.bashrc 2>/dev/null || true
...
if [[ ${#MISSING[@]} -gt 0 ]]; then
    ...
    echo "   Some steps may fail. Continue anyway? [y/N]"
    read -r response
    [[ "$response" != "y" ]] && exit 1
fi
...
# --- Step 4: Download large files from B2 ---
echo "=== Step 4: Downloading files from Backblaze B2 ==="
...
echo "=== Step 5: Authenticating services ==="
