...
DOC_FILE="/home/coder/nlp_Assignment3/experiments/t5-base-training-run.md"
    ...
    if [[ "$curr_epoch_line" != "$prev_epoch_line" ]]; then
        ...
        if [[ -z "$stall_start" ]]; then
            ...
            if [[ $elapsed -gt $STALL_THRESHOLD ]]; then
                ...
                append_note "Monitor: training appears stalled (no new epoch for ${elapsed}s)."
                stall_start=""  # Reset to avoid spamming
            fi
        fi
    fi

    # 7) Log GPU status periodically
    gpu_info=$(check_gpu)
    if [[ -n "$gpu_info" ]]; then
