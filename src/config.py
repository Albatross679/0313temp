...
class SLNeuralConfig(BaseConfig):
    ...
    # Mixed precision
    use_amp: bool = True              # bf16 autocast (Ampere+ GPU required)

    # Evaluation frequency
    eval_every_n_epochs: int = 1          # run eval every N epochs (1 = every epoch)

    # Eval-time speedups
    eval_num_beams: Optional[int] = None  # beams during training eval (None → use num_beams)
    eval_subset_size: Optional[int] = None  # eval on N dev examples during training (None → full)
    beam_early_stopping: bool = True      # stop beam search when num_beams hypotheses complete

    # Early stopping
    patience_epochs: int = 0              # 0 disables; counts eval cycles, not training epochs
    patience_tolerance: float = 0.0       # min improvement to reset patience (e.g. 0.001)
