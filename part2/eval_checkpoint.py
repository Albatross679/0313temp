#!/usr/bin/env python3
"""Load a T5 from-scratch checkpoint from an output dir and evaluate on dev examples.

Usage examples:
    # Auto-find the latest output directory
    python part2/eval_checkpoint.py

    # Specify an output directory directly
    python part2/eval_checkpoint.py --run_dir output/t5_scr_20260301_120000

    # Show more examples
    python part2/eval_checkpoint.py --num_examples 20

    # Use last checkpoint instead of best
    python part2/eval_checkpoint.py --use_last
"""
...
from part2.model import load_model_from_checkpoint
from part2.data import get_dataloader, _TOKENIZER
from utils import compute_metrics, save_queries_and_records

DB_PATH = "data/flight_database.db"


def find_latest_output_dir(prefix="t5_scr"):
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate a T5 from-scratch checkpoint")
    parser.add_argument("--run_dir", type=str, default=None,
                        help="Path to output run directory (e.g. output/t5_scr_20260301_120000)")
    ...
    if run_dir is None:
        run_dir = find_latest_output_dir("t5_scr")
    ...
    schema_mode = cfg.get("schema_mode", "tables")
    ...
    save_queries_and_records(all_preds, tmp_pred_sql, tmp_pred_pkl,
                             num_threads=cfg.get("sql_num_threads", 32))
